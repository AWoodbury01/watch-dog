import React, { Component } from "react";
import {
  Header,
  Container,
  Button,
  Icon,
  Image,
  Segment,
  Divider,
  Item,
  Grid,
  List,
} from "semantic-ui-react";
import "../profile/Profile.css";

class Home extends Component {
  render() {
    return (
      <>
        <Container text fluid>
          <Header
            as="h1"
            style={{
              fontSize: "4em",
              fontWeight: "normal",
              marginBottom: "4em",
              marginTop: "4em",
            }}
          >
            <Image src="data:image/png;base64," />
            Watch Dog
          </Header>
        </Container>
        <br />
        <br />
        <Container>
          <Header
            as="h3"
            textAlign="center"
            style={{
              fontSize: "3em",
              fontWeight: "normal",
              marginTop: "7em",
              marginBottom: "1em",
            }}
          >
            Search movies and TV shows from top providers
          </Header>
          <br />

          <Segment>
            <Container>
              <Image
                src="https://miro.medium.com/max/6126/1*sQlpBShXS5fktfHvuR6RIQ.jpeg"
                centered
                fluid
              />
            </Container>
            <Divider />
            <Header
              as="h2"
              style={{
                fontSize: "1.7em",
                fontWeight: "normal",
                marginTop: "1.5em",
                marginBottom: "1em",
              }}
            >
              Plan your next binge!
            </Header>
            <Button
              primary
              size="huge"
              href="/profile"
              style={{
                marginBottom: "1em",
              }}
            >
              Get Started
              <Icon name="right arrow" />
            </Button>
          </Segment>
        </Container>
        <Container>
          <Header
            as="h2"
            style={{
              fontSize: "1.7em",
              fontWeight: "normal",
              marginTop: "1.5em",
              marginBottom: "1em",
            }}
          >
            User Reviews
          </Header>
          <Segment style={{ marginBottom: "2em" }}>
            <Item.Group>
              <Item>
                <Item.Image
                  size="tiny"
                  src="https://react.semantic-ui.com/images/avatar/large/stevie.jpg"
                />

                <Item.Content>
                  <Item.Header>Roxanne Gonzales</Item.Header>
                  <Item.Description>
                    {" "}
                    "With Watch Dog, I know exactly what shows I've watched!"{" "}
                  </Item.Description>
                  <Item.Description>
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star outline" />
                  </Item.Description>
                </Item.Content>
              </Item>

              <Item>
                <Item.Image
                  size="tiny"
                  src="https://react.semantic-ui.com/images/avatar/large/justen.jpg"
                />

                <Item.Content>
                  <Item.Header>Corey Cortez</Item.Header>
                  <Item.Description>"Just wow"</Item.Description>
                  <Item.Description>
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                  </Item.Description>
                </Item.Content>
              </Item>

              <Item>
                <Item.Image
                  size="tiny"
                  src="https://react.semantic-ui.com/images/avatar/large/jenny.jpg"
                />

                <Item.Content>
                  <Item.Header>Essa Figueroa</Item.Header>
                  <Item.Description>
                    "This is literally the best thing"
                  </Item.Description>
                  <Item.Description>
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star" />
                    <Icon size="small" name="star outline" />
                  </Item.Description>
                </Item.Content>
              </Item>
            </Item.Group>{" "}
          </Segment>
        </Container>
        <Segment inverted vertical style={{ padding: "5em 0em" }}>
          <Container>
            <Grid divided inverted stackable>
              <Grid.Row>
                <Grid.Column width={3}>
                  <Header inverted as="h4" content="About" />
                  <List link inverted>
                    <List.Item as="a">Sitemap</List.Item>
                    <List.Item as="a">Contact Us</List.Item>
                    <List.Item as="a">This is NOT a link</List.Item>
                    <List.Item as="a">YouTube binges</List.Item>
                  </List>
                </Grid.Column>
                <Grid.Column width={3}>
                  <Header inverted as="h4" content="Important Sites" />
                  <List link inverted>
                    <List.Item as="a">Digital Can Opener Pre-Order</List.Item>
                    <List.Item as="a">Why does Tik Tok exist?</List.Item>
                    <List.Item as="a">Types of cast iron pans</List.Item>
                    <List.Item as="a">
                      Favorite episodes of 'The Office'
                    </List.Item>
                  </List>
                </Grid.Column>
                <Grid.Column width={7}>
                  <Header as="h4" inverted>
                    Topics
                  </Header>
                  <p>Theories on Who Let the Dogs Out</p>
                  <p>So.. How do you code exactly?</p>
                </Grid.Column>
                <Grid.Column>
                  <div>
                    <Button circular color="facebook" icon="facebook" />
                    <Button circular color="twitter" icon="twitter" />
                    <Button circular color="linkedin" icon="linkedin" />
                    <Button circular color="google plus" icon="google plus" />
                  </div>
                </Grid.Column>
              </Grid.Row>
            </Grid>
          </Container>
        </Segment>
      </>
    );
  }
}

export default Home;
